...
class Encoder(nn.Module):
    ...
    def __init__(self):
        ...
        layer_defs = []
        layer_defs.append(submodules.conv_1d(_EMB_REDUCTION_FACTOR, _CONV_CHANNELS))
        for i in range(24):
            layer_defs.append(submodules.ResNetModule1d(_CONV_CHANNELS, _CONV_CHANNELS, kernel_size=3, pad=1, dilation=2**(i%8)))
        layer_defs.append(submodules.conv_1d(_CONV_CHANNELS, 2 * _EMB_DIM))
        self._model = nn.Sequential(*layer_defs)

    def forward(self, x):
        x = x.view([x.size(0), _EMB_REDUCTION_FACTOR, -1])
        embs = self._model.forward(x)
        return torch.split(embs, _EMB_DIM, dim=1)
...
class Decoder(nn.Module):
    ...
    def __init__(self):
        ...
        layer_defs = []
        layer_defs.append(submodules.conv_1d(_EMB_DIM, _CONV_CHANNELS))
        for i in range(24):
            layer_defs.append(submodules.ResNetModule1d(_CONV_CHANNELS, _CONV_CHANNELS, kernel_size=3, pad=1, dilation=2**(i%8)))
        layer_defs.append(submodules.conv_1d(_CONV_CHANNELS, _EMB_REDUCTION_FACTOR))
        self._model = nn.Sequential(*layer_defs)
...
class Model(nn.Module):
    ...
    def forward(self, instr, vocal):
        ...
        instr_emb1, instr_emb2 = self._encoder.forward(instr)
        mixed_emb1, mixed_emb2 = self._encoder.forward(instr + vocal)
        vocal_emb1, vocal_emb2 = self._encoder.forward(vocal)
        ...
        instr_pred = self._instr_decoder.forward(mixed_emb1)
        ...
        vocal_pred = self._vocal_decoder.forward(mixed_emb2)
    ...
    def predict(self, mixed):
        ...
        _, mixed_emb2 = self._encoder.forward(mixed)
        vocal_pred = self._vocal_decoder.forward(mixed_emb2)
...
class Generator(Network):
    ...
    def predict(self, data, summary_prefix=''):
        ...
        for i in range(0, data[0].shape[1], chunk_size):
            ...
            prediction_i = self.model.predict(data_i)[0].detach().cpu().numpy()
